...
# 此处要根据自己文件存放路径来改
image_path = "D:/Car_Type_Recognizition/"  # 数据集路径
train_dir = image_path + "train"  # 训练集路径
validation_dir = image_path + "val"  # 验证集路径

# 存放训练好的模型
if not os.path.exists("car_save_weights"):
    os.makedirs("car_save_weights")
...
with open('car_class_indices.json', 'w') as json_file:
...
model = GoogLeNet(im_height=im_height, im_width=im_width, class_num=car_types, aux_logits=True)  # 实例化模型
...
for epoch in range(1, epochs + 1):
    ...
    if test_loss.result() < best_test_loss:
        ...
        model.save_weights("./car_save_weights/car_myGoogLeNet.h5")  # 保存模型为.h5格式
